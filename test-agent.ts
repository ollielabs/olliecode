...
async function main() {
  ...
  const result = await runAgent({
    ...
    userMessage: "Can you search the nested files in the source code and describe what this application does?",
    history: [],
    ...
    onStepComplete: (step) => {
      const actionsSummary = step.actions.map(a => `${a.function.name}(${JSON.stringify(a.function.arguments)})`).join(", ");
      console.log(`[test] Step complete. Actions: ${actionsSummary}, Duration: ${step.durationMs}ms\n`);
    },
