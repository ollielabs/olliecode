...
program
  ...
  .option("-h, --host <host>", "ollama host to connect to", "http://127.0.0.1:11434")
  ...
  .action(async (options) => {
    ...
    const { model, host, session: sessionId, continue: continueSession } = options;
    ...
    if (sessionId) {
      ...
      if (!session) {
        console.error(`No previous session found for this project. Starting fresh.`);
      } else {
        initialSessionId = session.id;
