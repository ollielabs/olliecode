...
const inputSchema = z.object({
  pattern: z
    .string()
    .min(1, { message: 'Pattern must not be empty' })
...
export const globTool: ToolDefinition<typeof inputSchema, typeof outputSchema> =
  {
    name: 'glob',
    description:
      'Find files matching a glob pattern. Excludes node_modules, .git, and build directories. Useful for discovering source files across nested directories.',
    ...
    execute: async ({ pattern, cwd }) => {
      ...
      for await (const file of glob.scan({ cwd: searchDir, onlyFiles: true })) {
        ...
        const shouldExclude = EXCLUDED_DIRS.some(
          (dir) => file.includes(`${dir}/`) || file.startsWith(`${dir}/`),
        );
