...
const outputSchema = z.array(z.object({
  ...
  line: z.number(),
  content: z.string(),
})).describe("Array of matches with file path, line number, and matching line content");
...
export const grepTool: ToolDefinition<typeof inputSchema, typeof outputSchema> = {
  ...
  execute: async ({ pattern, filePattern, cwd }) => {
    ...
    for await (const filePath of glob.scan({ cwd: searchDir, onlyFiles: true })) {
      ...
      try {
        const fullPath = searchDir === "." ? filePath : `${searchDir}/${filePath}`;
