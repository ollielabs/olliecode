...
import { processToolCalls, type ToolProcessingResult } from "../src/agent/tool-processor";
import { SafetyLayer } from "../src/agent/safety";
...
async function runParallelTest(): Promise<void> {
  ...
  console.log("Test 1: Multiple safe tools (read_file, glob, grep) run in parallel");
  {
    ...
    if (result.parallelCount === 3 && result.sequentialCount === 0) {
      console.log(`  ✅ PASS: ${result.parallelCount} tools ran in parallel (${elapsed}ms)`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected 3 parallel, got ${result.parallelCount} parallel, ${result.sequentialCount} sequential`);
      failed++;
    }
  }
  ...
  console.log("\nTest 2: Unsafe tools (run_command, write_file) run sequentially");
  {
    ...
    if (result.sequentialCount === 2 && result.parallelCount === 0) {
      console.log(`  ✅ PASS: ${result.sequentialCount} tools ran sequentially`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected 2 sequential, got ${result.parallelCount} parallel, ${result.sequentialCount} sequential`);
      failed++;
    }
  }
  ...
  console.log("\nTest 3: Mixed safe and unsafe tools");
  {
    ...
    if (result.parallelCount === 3 && result.sequentialCount === 1) {
      console.log(`  ✅ PASS: ${result.parallelCount} parallel, ${result.sequentialCount} sequential`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected 3 parallel + 1 sequential, got ${result.parallelCount} parallel, ${result.sequentialCount} sequential`);
      failed++;
    }
  }
  ...
  console.log("\nTest 4: Results maintain original order");
  {
    ...
    const orderCorrect = expectedOrder.every((name, i) => actualOrder[i] === name);
    
    if (orderCorrect) {
      console.log(`  ✅ PASS: Results in correct order: ${actualOrder.join(", ")}`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected ${expectedOrder.join(", ")}, got ${actualOrder.join(", ")}`);
      failed++;
    }
  }
  ...
  console.log("\nTest 5: Partial failures handled gracefully");
  {
    ...
    if (result.observations.length === 3) {
      ...
      console.log(`  ✅ PASS: All 3 tools returned results (${failures.length} with errors)`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected 3 observations, got ${result.observations.length}`);
      failed++;
    }
  }
  ...
  {
    const toolCalls: ToolCall[] = [
      createToolCall("task", { description: "test", prompt: "test prompt", thoroughness: "quick" }),
      createToolCall("read_file", { path: "package.json" }),
    ];
    ...
    if (result.parallelCount === 2) {
      console.log(`  ✅ PASS: task tool runs in parallel with other safe tools`);
      passed++;
    } else {
      console.log(`  ❌ FAIL: Expected 2 parallel, got ${result.parallelCount} parallel, ${result.sequentialCount} sequential`);
      failed++;
    }
  }
