...
async function testPrompt(
  ...
  try {
    ...
    if (
      /```(?:typescript|javascript)\s*\n.*(?:read_file|list_dir|run_command)/s.test(
        answer,
      )
    ) {
    ...
    // Check for refusals
    if (
      /\b(cannot|can't|unable to|don't have access)\b/i.test(answer) &&
...
function printResult(result: TestResult) {
  ...
  console.log(
    `   Tools: ${tools} | Answer: ${result.answerLength} chars | Time: ${result.duration}ms`,
  );
...
function printSummary(results: TestResult[]) {
  ...
  const lazy = results.filter((r) =>
    r.notes.some((n) => n.startsWith('LAZY')),
  ).length;
  const codeOutput = results.filter((r) =>
    r.notes.some((n) => n.startsWith('OUTPUT_CODE')),
  ).length;
  const refused = results.filter((r) =>
    r.notes.some((n) => n.startsWith('REFUSED')),
  ).length;
  ...
  console.log(
    `Used tools:      ${withTools} (${Math.round((withTools / total) * 100)}%)`,
  );
  ...
  for (const [cat, catResults] of byCategory) {
    const catSuccess = catResults.filter(
      (r) => r.success && r.notes.length === 0,
    ).length;
...
async function main() {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--model' && args[i + 1]) {
      ...
      category = (args[i + 1] ?? null) as Category;
      i++;
    } else if (args[i] === '--prompt' && args[i + 1]) {
      singlePrompt = args[i + 1] ?? null;
      i++;
    } else if (args[i] === '--all') {
      runAll = true;
    }
  ...
  if (singlePrompt) {
    ...
    console.log('Usage:');
    console.log(
      '  --model <name>     Model to test (default: granite4:latest)',
    );
