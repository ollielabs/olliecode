...
async function testSimpleCompaction() {
  ...
  const result = await compactMessages(
    messages,
    "light",
    { ...DEFAULT_COMPACTION_CONFIG, useLLMSummary: false }
  );
  
  console.log("Messages after:", result.compactedCount);
  console.log("Tokens after:", result.tokensAfter);
  console.log("Reduction:", Math.round((1 - result.tokensAfter / result.tokensBefore) * 100) + "%");
  ...
  console.log("Last 6 messages roles:", lastMessages.map(m => m.role).join(", "));
