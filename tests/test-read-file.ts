...
async function runTests(): Promise<void> {
  ...
  console.log('Test 1: Line numbers are added to output');
  {
    ...
    if (hasLineNumbers && hasMultipleLineNumbers) {
      console.log(
        `  ✅ PASS: Output contains ${lineNumberMatches.length} line numbers`,
      );
      passed++;
    } else {
  ...
  console.log('\nTest 2: Output wrapped in <file> tags');
  {
    ...
    if (hasOpenTag && hasCloseTag) {
      ...
      console.log(
        `  ❌ FAIL: Missing file tags (open: ${hasOpenTag}, close: ${hasCloseTag})`,
      );
      failed++;
    }
  }
  ...
  {
    // fullResult unused - we only need offsetResult to verify offset works
    await readFileTool.execute({ path: 'package.json' });
    const offsetResult = await readFileTool.execute({
      path: 'package.json',
      offset: 5,
    });
  ...
  console.log('\nTest 5: Offset and limit work together');
  {
    ...
    const lineNumbers = (result.match(/^\s*(\d+)\|/gm) || []).map((m) =>
      parseInt(m.replace(/[^\d]/g, ''), 10),
    );

    const expected = [4, 5, 6, 7];
    const matches =
      lineNumbers.length === 4 &&
      lineNumbers.every((n, i) => n === expected[i]);

    if (matches) {
      console.log(
        `  ✅ PASS: Lines ${lineNumbers.join(', ')} returned correctly`,
      );
      passed++;
    } else {
      console.log(
        `  ❌ FAIL: Expected lines 4-7, got ${lineNumbers.join(', ')}`,
      );
      failed++;
    }
  }
  ...
  console.log('\nTest 6: Path included in file tag');
  {
    ...
    const hasPath =
      result.includes('<file path="') && result.includes('package.json');
  ...
  console.log('\nTest 8: Line number padding is consistent');
  {
    ...
    if (widths.size === 1) {
      ...
      console.log(
        `  ❌ FAIL: Inconsistent padding widths: ${[...widths].join(', ')}`,
      );
      failed++;
    }
  }
